...
class FFNN:
    ...
    def fit(
        ...
        Description:
        ------------
            This function performs the training the neural network by performing only the feedforward
            algorithm to update the networks weights, functioning as a logistic regression.
...
for i, etaa in enumerate(eta_vals):
    for j, lmbd in enumerate(epochs): # starts a loop to test variables.
        ...
        FFNN1.fit(X_train,y_train, Adam(eta = (etaa), rho=0.9, rho2=0.999), epochs = lmbd, lam = 0.1)
