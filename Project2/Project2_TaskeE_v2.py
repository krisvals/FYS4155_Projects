# -*- coding: utf-8 -*-
"""
Created on Thu Nov  9 12:45:04 2023
...
M = 10
lmbd_vals = [1E-4, 1E-3,1E-2,1E-1] 
eta_vals = [1E-4, 1E-3,1E-2,1E-1]  #Can be adjusted, but code gets very slow for eta below 1e-10
epochs = 100
...
for i, etaa in enumerate(eta_vals):
    for j, lmbd in enumerate(lmbd_vals): # starts a loop to test variables.
        FFNN1.reset_weights()
        FFNN1.fit(X_train,y_train, Adagrad(eta = (etaa)), epochs = 1000, lam = lmbd) #train data to the fit. Using the training data here, not sure if this is the best option. Many parameters can be set to optimize the fit, such as eta, lambda, momentum.
