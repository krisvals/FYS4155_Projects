...
FFNN1 = FFNN(dimensions = [n_input_nodes,n_hidden_nodes,n_output_nodes],hidden_func = RELU, output_func=softmax, cost_func=CostCrossEntropy) #creates an object of the neural network
...
for i, etaa in enumerate(eta_vals):
    for j, lmbd in enumerate(epochs): # starts a loop to test variables.
        ...
        FFNN1.fit(X_train,y_train, Adagrad(eta = (etaa)), epochs = lmbd, lam = 0.01)
